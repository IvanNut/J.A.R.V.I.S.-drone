...
def va_respond(voice: str):
    ...
    if len(cmd['cmd'].strip()) <= 0:
        ...
        # Проверяем, если сказано "Выполни", то подключаем GPT для генерации кода
        if fuzz.ratio(voice.split()[0], "выполни") > 75:
            message_log.append({"role": "user", "content": f"Только код на Python для управления с двумя дронами дроном Tello, используй библиотеку from djitellopy import TelloSwarm: {voice}. используй swarm = TelloSwarm.fromIps(['192.168.0.120','192.168.0.121', '192.168.0.158', '192.168.0.184']). Ответ без ```."})
            
            # Получаем код от ChatGPT
            response = gpt_answer()  
            ...
            tts.va_speak("Код сгенерирован и запущен")
            time.sleep(0.5)
...
def execute_cmd(cmd: str, voice: str):
    ...
    elif cmd == 'connect_drones':
        ssid = "drone"  # Здесь можно динамически подставлять SSID
        password = "12345678"  # Аналогично с паролем
